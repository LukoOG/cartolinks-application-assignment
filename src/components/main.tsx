  ...
  return (
    <section className="flex flex-col h-full overflow-x-hidden">
        ...
        <Carousel className="w-full h-full block" setApi={setApi}>
          <CarouselContent className="min-h-full">
            {images.map((url, index) => (
              <CarouselItem
                key={index}
                className="max-h-fit shrink-0 w-full sm:basis-7/12"
              >
                <div className="p-1">
                  <div className="bg-card text-card-foreground rounded-xl shadow-sm overflow-hidden border max-h-fit w-full">
                    <Image
                      ...
                      width={1200}
                      height={800}
                      className="w-full h-90 aspect-3/2 object-fit"
                      src={url}
                    />
                  </div>
                </div>
          ...
          {/* Controls row */}
          <div className="grid grid-cols-[1fr_1fr_1fr] sm:grid-cols-[28rem_1fr_1fr] items-center justify-between mt-2 px-2">
            ...
            <div className="col-start-2 span-1 items-center justify-center flex gap-2 min-w-full max-w-fit h-full">
              {images.map((_, index) => (
                <span
                  ...
                  className={`${selectedIdx === index + 1 ? "bg-indicator-foreground" : "bg-indicator"} h-2 w-2 rounded-full cursor-pointer`}
                ></span>
              ))}
            </div>
      ...
      <div className="min-h-full">
        <h2 className="max-h-fit w-full text-left font-600 text-lg sm:text-xl">
          Generate
        </h2>
        <section className="grid grid-rows-2 grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-2 justify-evenly min-h-full">
